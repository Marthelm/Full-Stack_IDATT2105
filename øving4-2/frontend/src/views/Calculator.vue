...
<script>
import { sendExpression } from "../service/api.js"
import {getLastExpression} from "../service/api.js"
export default {
  data() {
    return {
      previous: null,
  ...
  methods: {
    ...
    async equal() {
      let temp = this.current; 
      let ex = await sendExpression(this.previous,temp,this.operatorSign);
      this.current = ex.data.solution
      let result = await getLastExpression();
